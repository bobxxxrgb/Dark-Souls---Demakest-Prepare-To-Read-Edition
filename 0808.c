#include <iostream>  //first asylum demon implementation
#include <fstream>
#include <string>
#include <chrono>
#include <thread>
...
case 'f':
  ...
  else (cout<<endl<<endl<<"(You)"<<opponent<<" can't be parried."<<endl);
    }  //if (hostile >= 3)
  else (cout<<endl<<endl<<"(You)"<<opponent<<" isn't attacking."<<endl);
   }  //if (stm-5>=0)
  else (cout<<endl<<endl<<"(You)You don't have enaugh stamina."<<endl);
   if (hostile >= 3) {
   oppatt = 0 + rand() % 4; }
  ...
  case 'G':                                             //estus
  case 'g':
   cout<<endl<<endl<<"(You)You made spacing to get the right opportunity to drink from your Estus flask."
   <<endl<<"(You)You recover HP and stamina."<<endl;
   if(hp<=maxhp-500){
   hp=hp+500;}
   else(hp=maxhp);
     if(stm<=maxstm-15){
...
while (x=9) {
 if (world==0) {
  ...
  cin >> chose;
     ...
     else{cout<<endl<<"You found an item soul in one of them!"<<endl<<endl;Asylum_Demon();}
     break;
     case 'e':
     case 'E':
     if(asyldemfight=0)
