...
void bonfire(){
    if(gotolord==0){
     ...
     switch(chose);
     {
      ...
      if(maxestus<40){maxestus=maxestus+5;humanity=humanity-1;cout<<endl<<"You Kindled and your Estus flask can now hold more Flames.";}else{cout<<endl<<"You can't Kindle any further.";}
      }
      }
...
petitelondo= 0;
  ...
  nextlevel= 0;
   ...
   oppsouls= 0;
    ...
    ifstream sv_20 ("sv_20");
    if(sv_20.is_open())
    {
     while (getline(sv_20,catchsave))
     maxestus=stoi(catchsave)/95610;
    {
    sv_20.close();
   }}
...
save();    
  ...
  cout << "'Undead Asylum'  'Upwards Corridor'"<<endl<<"Souls: "<<souls<<endl<<hp<<"HP    "<<stm<<"STM"
  <<endl<<endl<<"Select where to go:"<<endl<<
   "(W) To balcony"<<endl<<"(A) Check cell"<<endl<<"(S) Back to Bonfire"<<endl<<"(D)"<<endl<<"(Q) Check corpse in the middle of the stairs"<<endl<<"(E) Inventory"
  <<endl<<endl<<"input: ";
  cin >> chose;
     switch(chose) {
     case 's':
