...
case 'a':
if (stm-10>=0) {
  cout<<endl<<"                                          (You)You attacked "<<opponent<<" with a sword swing!"<<endl<<endl;
  opphp = opphp-atk*30/oppdef;
  stm=stm-10;
  }  //if (stm-10>=0)
  else (cout<<"                                          (You)You don't have enaugh stamina."<<endl<<endl);
if (hostile >= 3) {
  oppatt = 0 + rand() % 4; }
  else (oppatt = 0);
  break;
...
case 'b':
  if (stm-17>=0) {  
  cout<<"                                          (You)You attacked "<<opponent<<" with a heavy sword swing!"<<endl<<endl;
...
case 'C':                                                                  //kick
case 'c':
  cout<<"                                          (You)You kicked "<<opponent<<"!"<<endl<<endl;
  opphp = opphp-1;
    if (stm<maxstm-10){
    stm = stm+10;}
    else (stm=maxstm);
...
case 'd':
  if (stm-10>=0) {
  cout<<"                                          (You)You ran past the enemy."<<endl<<endl;
  fight = 0;
  break;
  }  //if (stm-10>=0)
else (cout<<"                                          (You)You don't have enaugh stamina."<<endl<<endl);
...
case 'e':
  if (stm-5>=0) {
  cout<<"                                          (You)You raised your shield waiting for "<<opponent<<" to attack."<<endl<<
  "                                          (You)your defense rose."<<endl<<endl;
  def=def+60;
    if (stm>0){stm=stm-5;}
     if (hostile >= 3) {
     oppatt = 0 + rand() % 4; }
...
else (cout<<"                                          (You)You don't have enaugh stamina.");
  if (hostile >= 3) {
  oppatt = 0 + rand() % 4; }
  else (oppatt = 0);
  break;
...
case 'f':
  if (stm-5>=0) {
  cout<<"                                          (You)You tried to parry "<<opponent<<"'s attack."<<endl;
  if (hostile == 3) {
  if (parry == 1) {
  parry2=0+rand() % 10;
  if (parry2 == 0)
  ...
  else (cout<<"                                          (You)"<<opponent<<" can't be parried.");
  }  //if (hostile == 3)
  else (cout<<"                                          (You)"<<opponent<<" isn't attacking.");
  }  //if (stm-5>=0)
  else (cout<<"                                          (You)You don't have enaugh stamina.");
   if (hostile >= 3) {
   oppatt = 0 + rand() % 4; }
  ...
  case 'G':                                                                      //estus
  case 'g':
   cout<<"                                          (You)You made spacing to get the right opportunity to drink from your Estus flask."
   <<endl<<"                                          (You)You recover HP and stamina."<<endl<<endl;
   if(hp<=maxhp-500){
   hp=hp+500;}
   else(hp=maxhp);
     if(stm<=maxstm-15){
...
switch(oppatt) {
case 0:                                                                                   //spacing
  cout<<endl<<endl<<"                                          (Enemy)"<<opponent<<" is spacing."<<endl<<endl;
  hostile++;
  break;
case 1:                                                                                   //slash
  cout << "                                          (Enemy)"<<opponent<<" Attacked you with a slash of their "<<weapon<<endl<<endl;
  hp=hp-oppatk*400/230;
  break;
case 2:                                                                                   //heavy
  cout << "                                          (Enemy)"<<opponent<<" Attacked you with a heavy slash of their "<<weapon<<endl<<endl;
  hp=hp-oppatk*500/230;
break;
case 3:                                                                                   //defense
cout << "                                          (Enemy)"<<opponent<<" Used their "<<weapon<<" to defend theirself"<<endl<<endl; 
break;
   }   //switch(oppatt)
 }     //for (fight=1;fight==1;x==9)
}      //void battle()
